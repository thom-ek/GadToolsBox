...
/* Includes */
	...
	FPuts(handle,"#include<gadtoolsbox/gadtoolsbox.h>
...
/* Global variables - available only if screen generation is on */
	if(OPT_Screen)
	{
		FPuts(handle,"Object *MainScr=NULL;\nstruct Screen *Scr=NULL;\n");
		...
		{
			FPrintf(handle,"ULONG ScrTags[]=\n{\n"\
										 "	SCRN_LockPubName,%s,\n"\
										 "	TAG_DONE\n};\n",ns->ns_Public[0]?"ScrPubName","NULL");
		}
		else
		{
			FPrintf(handle,"ULONG ScrTags[]=\n{\n"\
										 ...
										 "	SCRN_Type, %s,\n",ns->ns_Width,ns->ns_Height,ns->ns_Depth,ns->ns_MyFlags&MYSC_DefaultID?0x0:ns->ns_DisplayID,
														(ns->ns_Type==TYPE_CUSTOMSCREEN)?"CUSTOMSCREEN","PUBLICSCREEN,\n	SCRN_PubName, ScrPubName");
			GetFlag(ns->ns_MyFlags,MYSC_LikeWB) FPuts(handle,"	SCRN_LikeWorkbench, TRUE,\n");
...
/* Message ports for App and Project windows */
	if(CheckProjectWindow()) FPuts(handle,"Object *ProjectMsgPort=NULL;\n");
...
/* Windows releated definitions */
	for(wo=(piObject *)GetHead(List(&GUI->gui_List_Window));GetSucc(wo);wo=(piObject *)GetSucc(wo))
	{
		...
		FPrintf(handle,"\n/* %s Window structures */\n",nw->nw_Label);
		if(nw->nw_PageGadget==0) FPrintf(handle,"Object *%sWnd=NULL;\n",nw->nw_Label);
		if(!IsListEmpty(List(&nw->nw_List_Gadget))) FPrintf(handle,"struct Gadget *%sGList=NULL;\n",nw->nw_Label);
		if(!IsListEmpty(List(&nw->nw_List_Gadget))) FPrintf(handle,"Object *%sGadgets[CNT_%s];\n",nw->nw_Label,nw->nw_Label);
	}
	FPuts(handle,"\n");
...
*/
	...
	FPrintf(handle,"#define VER_TITLE \"%s v%ld.%ld\"\n",nv->nv_ProgramName,nv->nv_Version,nv->nv_Revision);
	FPrintf(handle,"#define VER_EXTENSION \"rx\"\n");
	...
	if(OPT_Screen)
	{
		...
		if(ns->ns_Type!=TYPE_WORKBENCH);
		FPuts(handle,"extern Object *Scr;\n");
	}
...
/* Windows releated definitions */
	if(!IsListEmpty(List(&GUI->gui_List_Window)))
	{
		for(cnt=0,wo=(piObject *)GetHead(List(&GUI->gui_List_Window));GetSucc(wo);wo=(piObject *)GetSucc(wo),cnt++)
		{
			...
			if(nw->nw_PageGadget==0) FPrintf(handle,"extern Object *%sWnd;\n",nw->nw_Label);
			...
			if(!IsListEmpty(List(&nw->nw_List_Gadget))) FPrintf(handle,"extern Object *%sGadgets[];\n",nw->nw_Label);
...
//	}
	...
	for(wo=(piObject *)GetHead(List(&GUI->gui_List_Window));GetSucc(wo);wo=(piObject *)GetSucc(wo))
	{
		...
		if(nw->nw_PageGadget==0)
		{
			FPrintf(handle,"ULONG HandleIDCMP_%s(Object *,ULONG,APTR);\n",nw->nw_Label);
...
{
	SavePrefs("ENV:GadToolsBox/boopsi-c.generator.prefs");
	SavePrefs("ENVARC:GadToolsBox/boopsi-c.generator.prefs");
	return 0;
}
...
#define ID_CGEN MAKE_ID('B','C','G','E')
